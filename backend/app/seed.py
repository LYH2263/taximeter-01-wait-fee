...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS wait_fee_rules(id INTEGER PRIMARY KEY, label TEXT NOT NULL, free_min REAL NOT NULL, per_min REAL NOT NULL, active INTEGER NOT NULL DEFAULT 1, created_at TEXT);
    """)
    if conn.execute("SELECT COUNT(*) c FROM tariff").fetchone()["c"] == 0:
        ...
        conn.execute("INSERT INTO wait_fee_rules(label,free_min,per_min,active,created_at) VALUES ('标准等候',3,0.5,0,datetime('now'))")
        ...
        conn.execute("INSERT INTO calc_runs(kind,trip_id,input_json,result_json,created_at) VALUES ('fare',1,?,?,datetime('now'))",
            (json.dumps({"distance_km":5,"slow_min":2,"wait_min":0,"night":False}), json.dumps(r)))
        conn.commit()
    conn.close()
